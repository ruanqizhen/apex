...
export interface GameStoreActions {
  ...
  onLevelUp?: () => void;
  onGameOver?: () => void;
  triggerActiveSkill?: () => void; // 主动技能释放
  cheatGainMass?: () => void;      // 开发者测试作弊键
}
...
export interface Player extends BaseEntity {
  ...
  inkCooldownUntil: number | null; // 主动技能墨汁冷却截止逻辑时钟时间戳
}
...
export interface WorldState {
  ...
  upgradeAnimationTimer: number | null; // 升级动画截止逻辑时钟
  upgradeAnimationType: 'tadpole_to_fry' | 'fry_to_juv' | 'juv_to_pred' | 'pred_to_levi' | null; // 升级大类
...
| 道具 Item | 固定半径 14 | 玩家吞噬后获得增益 | 极其缓慢地漂移（baseSpeed = 0.08） |
...
* **特殊掠食者 AI 覆盖**：
  1. **变色巨乌贼（Predator, speciesIndex = 3）**：拥有主动隐形伪装技术。当它与玩家的距离大于 250 时，其透明度 `globalAlpha` 降为 0.05（几乎完全隐形）；在距离 150 到 250 之间时，根据距离差线性渐变显形；当距离小于 150，或者其 AI 状态进入 `Pursue` 或 `Attack` 时，透明度会强制恢复到至少 0.7 显形，提高偷袭紧张感。
  2. **冲撞剑鱼（Predator, speciesIndex = 4）**：具有蓄力直线突刺技能。其行为状态机被扩展为三个子阶段：
     * `normal`：普通巡游。首次相遇的前 2 秒处于冷却，随后每 4 秒（4000ms CD）若探测到玩家并处于 `Pursue` 或 `Attack` 状态，将锁定玩家当前的瞬时坐标，并进入 `charging` 阶段。
     * `charging`：朝着锁定的坐标点发起直线冲撞，持续 1.0 秒（1000ms），冲刺游速提升至其基础速度的 3.5 倍，朝向强制锁定，并在身后留下黄金水泡尾迹。
     * `stunned`：冲刺结束后，进入眩晕脱力状态，持续 1.5 秒（1500ms），此期间它的速度骤减至 `baseSpeed * 0.15`，沿原方向惯性缓慢滑行，且在此期间不会转弯或追击。

### 6.4 动态难度与生成回收控制

* **动态难度曲线**：为了保证游戏后期挑战性，AI 的最大生成上限、移动速度和感知范围将随着玩家的 `evolutionLevel`（等级）动态攀升：
  - **掠食者 Predator 数量**：从 Lvl 0 初始 of 1 只增加至最高 **16 只**（公式：`Math.min(16, 1 + level * 2)`），中后期生存压力陡增。
  - **竞争者 Competitor 数量**：从 5 只增加至最高 **30 只**（公式：`Math.min(30, 5 + level * 3)`），增加战场弹性阻碍。
  - **猎物 Prey 数量**：开局 60 只便于玩家成长，后期逐步收紧至最低 **25 只**（公式：`Math.max(25, 60 - level * 3)`），加大捕猎难度。
  - **掠食者属性成长**：其感知半径随玩家等级成长为 `radius * Math.min(12.0, 6.0 + level * 0.6)`；基础游速随玩家等级从 `0.5` 倍成长至最高 `1.15` 倍（公式：`Math.min(1.15, 0.5 + level * 0.05)`）。
* **生成时机与位置**：环境生态在 **Camera 视口外环形区域**（半径为当前视口世界对角线的 `[0.6, 1.0]` 倍）随机取点。每次生成前进行 **重叠检查**（最多重试 5 次），如果与玩家或已有实体距离小于双方半径之和则重新取点，5次失败后放弃本次生成。
* **回收**：实体与玩家距离超过视口世界对角线的 `2.5` 倍时，标记 `isAlive = false`，移出空间哈希并归还对象池（`entityPool.releaseAIEntity(entity)`），规避频繁 `new` 的 GC 阻碍。

### 6.5 吞噬/被吞噬结算（Consumption System）

* **吞噬判定检测**：每 tick 对玩家做一次空间哈希临近查询（范围为 `player.radius * 3.0`），收集所有重叠实体（距离小于 `player.radius + entity.radius * eatEngulfMultiplier`，其中 `eatEngulfMultiplier` 受"深渊巨口"突变和 Frenzy 状态加成）。
* **多实体碰撞结算顺序**：收集到所有碰撞候选实体后，**按其半径升序排序**，依次执行吞噬结算，并立即更新 `player.radius` 计算下一个实体碰撞，避免过期半径导致穿透或判定错误。
* **吞噬普通生物**：
  - 玩家吞噬 Plankton 或 Prey 时，根据 `EAT_EFFICIENCY` 及“高效消化”突变乘数累加质量，更新 `player.radius`，累加 `comboCount`，更新 `comboLastEatAt`。
  - 触发 `eat_burst` 品种特定色爆发粒子，以及 `eaten_prey` 缩水吸入挣扎粒子。
* **吞噬道具 (Item)**：
  - 吞噬道具时不改变玩家质量与体积，直接开启对应的 Magnet（磁力）、Freeze（冰冻）或 Shield（护盾）效果，触发 `item_pickup` 对应色粒子。
* **被吞噬（致命结算）**：
  - 若重叠的 Predator 满足致命阈值（`player.radius < predator.radius * 0.77`），且玩家当前不处于无敌状态，进入致命清算：
    1. **气泡护盾（Bubble Shield）优先消耗**：若 `player.shieldActive === true`，将其置为 `false`，玩家获得 1.5 秒（1500ms）无敌时间，向反方向弹开 `predator.radius + player.radius + 60` 距离，弹开速度为 `player.baseSpeed * 2.5`，触发 `shield_break` 气泡碎裂粒子。不触发死亡。
    2. **骨化重甲（mut_shield）二次消耗**：若无气泡护盾但存在 `mut_shield`，扣除 1 层，玩家获得 1.0 秒（1000ms）无敌时间，反向弹开 `predator.radius + player.radius + 50` 距离，速度为 `player.baseSpeed * 2.0`，触发 `shield_break` 特效。不触发死亡。
    3. **无盾致死**：若以上两个防线皆无，则 `player.isAlive = false`，`status = "game_over"`，触发 `onGameOver()`。
* **物理碰撞弹开**：
  - 同级 Competitor，或者大小相似无法互相吞噬的实体（半径比例在 `[0.8, 1.25]` 之间），执行 **弹性动量碰撞**。根据质量与速度分量，修正彼此位置（解除重叠）和反弹速度，不发生质量转移。
